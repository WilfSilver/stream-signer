...
/// use identity_iota::{
///     core::Url,
///     credential::{Credential, Jwt, PresentationBuilder, JwtPresentationOptions, Subject},
///     storage::{JwkDocumentExt, JwkMemStore, JwsSignatureOptions, KeyIdMemstore, Storage},
///     document::CoreDocument,
/// };
...
///         let pres = PresentationBuilder::new(self.document.id().to_url().into(), Default::default())
...
/// # };
/// #
/// # use testlibs::{issuer::TestIssuer, TestIdentity};
/// #
...
///
/// let chunk_signer = ChunkSigner::new(Timestamp::from_millis(100), signer);
///
/// // This is generated by the FrameManager based on this definition
/// let msg = vec![0, 0, 0, /* ... */]
/// let signature = chunk_signer.sign(msg, Vec2u::new(1920, 1080), 2).unwrap();
///
...
/// let signature = chunk_signer.sign(msg, Vec2u::new(1920, 1080), 2).unwrap();
///
/// assert!(matches!(signature.presentation, PresentationOrId::Def(_)));
/// # Ok(())
/// # }
/// ```
///
...
/// let signer = Arc::new(identity);
///
/// let chunk_signer = ChunkSigner::new(Timestamp::from_millis(100), signer).as_ref();
///
/// // This is generated by the FrameManager based on this definition
/// let msg = vec![0, 0, 0, /* ... */]
/// let signature = chunk_signer.sign(msg, Vec2u::new(1920, 1080), 2).unwrap();
///
/// assert!(matches!(signature.presentation, PresentationOrId::Ref(_)));
/// # Ok(())
/// # }
/// ```
///
...
impl<S: Signer> ChunkSigner<S> {
    ...
    pub const fn new(start: Timestamp, signer: Arc<S>) -> Self {
        Self {
            signer,
            pos: None,
            size: None,
